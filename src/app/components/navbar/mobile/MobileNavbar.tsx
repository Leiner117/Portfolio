...
const MobileNavbar = ({ onToggleTheme, theme, isScrolled }: { onToggleTheme?: () => void; theme?: 'light' | 'dark'; isScrolled?: boolean }) => {
  ...
  style={{ color: theme === 'dark' ? '#FFFFFF' : (theme === 'light' ? '#000000' : (!isScrolled ? '#FFFFFF' : undefined)) }}
    >
      {data.label}
    </a>
  );
    ...
    <nav className="relative w-full">
      <div className="flex items-center justify-between">
          ...
          <button 
            ...
            style={{ 
              color: theme === 'dark' ? '#FFFFFF' : (!isScrolled ? '#FFFFFF' : undefined),
          ...
          >
            <svg
              ...
              fill="none"
              viewBox="0 0 24 24"
              stroke={theme === 'dark' ? '#FFFFFF' : ((!isScrolled && '#FFFFFF') || (theme === 'light' ? '#000000' : undefined))}
            >
              {isMobileMenuOpen ? (
                <path
                  strokeLinecap="round"
